...
export default function PersonnelMessagesScreen() {
  ...
  return (
    <Screen scroll>
      <PageHeader
        title="Mesajlar"
        subtitle="Yönetimden gelen duyurular"
        module="dashboard"
        action={
          user.canMessageAdmins ? (
            <Button title="+ Yaz" onPress={() => router.push('/(personnel)/messages/compose')} />
          ) : undefined
        }
      />
      ...
      {isLoading ? (
        ...
        !inbox?.length ? (
          <Card>
            <Text style={styles.preview}>Henüz mesajınız yok.</Text>
          </Card>
        ) : (
          inbox.map((item) => (
            <Pressable
              ...
              onPress={() => {
                ...
                router.push({
                  pathname: '/(personnel)/messages/[id]',
        ...
        <Card>
          <Text style={styles.preview}>Henüz yöneticilere mesaj göndermediniz.</Text>
        </Card>
      ) : (
        sent.map((item) => (
          <Pressable
